...
const Main = () => {
  ...
  const { data: todos } = useQuery(
    ['todos'],
    async () => await getUserTodos(user?.token as string)
  )
    ...
    <section className='flex flex-col w-full max-w-md gap-5'>
      ...
      <div className='flex flex-col gap-3'>
        <div className='flex items-center justify-between gap-3'>
          ...
          <button
            className='flex-shrink-0 text-sm font-bold text-blue-500 underline border-none outline-none underline-offset-2 2xl:text-base'
            onClick={createTodo}
          >
            Add todo
          </button>
